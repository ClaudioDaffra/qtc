...
extern struct LibraryFunction qString[] ;

extern void StdLimitsSetupFunc(Picoc *pc) ;
extern void StdintSetupFunc(Picoc *pc) ;

extern struct LibraryFunction qWidget[] ;
extern void qWdigetSetupFunc(Picoc *pc) ;

extern struct LibraryFunction qPaint[] ;
extern void qPaintSetupFunc(Picoc *pc) ;

extern struct LibraryFunction qComplex[] ;
extern void qComplexSetupFunc(Picoc *pc) ;
...
{
	IncludeRegister(pc, "qComplex.h", &qComplexSetupFunc	, &qComplex[0]			, NULL);
    IncludeRegister(pc, "qPaint.h"	, &qPaintSetupFunc		, &qPaint[0]			, NULL);	
    IncludeRegister(pc, "qString.h"	, NULL					, &qString[0]			, NULL);
    IncludeRegister(pc, "limits.h"	, &StdLimitsSetupFunc	, NULL					, NULL);
    IncludeRegister(pc, "stdint.h"	, &StdintSetupFunc		, NULL					, NULL);	
    IncludeRegister(pc, "qWidget.h"	, &qWdigetSetupFunc		, &qWidget[0]			, NULL);	
    /**/
...
void IncludeRegister
(Picoc *pc, const char *IncludeName, void (*SetupFunction)(Picoc *pc), struct LibraryFunction *FuncList, const char *SetupCSource)
{
